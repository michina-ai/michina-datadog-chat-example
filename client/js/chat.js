...
const get_streaming_response = async (message) => {
...
const collect_stream = async (reader) => {
  let _text = "";
  while (true) {
...
const render_stream = async (reader) => {
  while (true) {
    const { value, done } = await reader.read();
    if (done) break;
...
const add_gpt_message_div = (gpt_image, token) => {
  message_box.innerHTML += `
<div class="message">
    <div class="user">
        ${gpt_image} <i class="fa-regular fa-phone-arrow-down-left"></i>
    </div>
    <div class="content" id="gpt_${token}">
        <div id="cursor"></div>
    </div>
</div>
`;
...
const add_user_message_div = (user_image, token, message) => {
  message_box.innerHTML += `
<div class="message">
    <div class="user">
        ${user_image}
...
const move_message_up = async (timeout) => {
...
const reset_message_input = () => {
  message_input.value = ``;
  message_input.innerHTML = ``;
  message_input.innerText = ``;
};

const ask_gpt = async (user_message) => {
  try {
    reset_message_input();

    add_conversation(window.conversation_id, user_message.substr(0, 20));
    window.scrollTo(0, 0);
    ...
    prompt_lock = true;
    window.text = ``;
    window.token = make_message_id();

    stop_generating.classList.remove(`stop_generating-hidden`);

    add_user_message_div(user_image, window.token, format(user_message));

    await move_message_up(500);

    add_gpt_message_div(gpt_image, window.token);

    await move_message_up(1000);

    const response = await get_streaming_response(user_message);
    const reader = response.body.getReader();
    await render_stream(reader);
    ...
    add_message(window.conversation_id, "user", user_message);
